...
- `最终一致性`：指**系统中的所有数据副本经过一定的时间后，最终达到一致的状态**，弱一致性和强一致性相反，最终一致性是弱一致性的一种特殊情况
...
+ synchronized关键字及线程之间的协作(wait、notify、notifyAll等)

+ CAS(Compare And Swap): 即比较替换，是实现并发应用到的一种技术。包含三个操作数 —— 内存位置(V）、预期原值(A)和新值(B), 如果内存位置上的值与预期原值相匹配，那么处理器会自动将该位置上的值更新为新值, 否则，处理器不做任何操作

+ 线程池: 通过复用线程可以大大减少由于线程频繁创建与销毁所带来的性能上的损耗
...
+ 数据库分库(垂直拆分和水平拆分)和分表
+ 数据库主从复制实现读写分离，解决大数据的查询问题
+ 利用Nosql，将MongoDB/PostreSQL配合MySQL组合使用
...
> 可能会面临的问题?
+ 用户请求量太大：单个服务器TPS(Transactions per second)，内存，I/O都是有限的 - 解决方法：分散请求到多个服务器上，堆机器就行
...
+ 分库分表后，单机数据库上的传统事务就变成分布式事务了，如果依赖数据库本身的分布式事务管理功能去执行事务，将付出高昂的性能代价。如果由应用程序去协助控制，形成程序逻辑上的事务，又会造成编程方面的负担
+ 跨库Join: 分库分表后表之间的关联操作将受到限制，我们无法直接Join位于不同分库的表，也无法Join分表粒度不同的表， 结果原本一次查询能够完成的业务，可能需要多次查询才能完成。粗略的解决方法：
	+  全局表：基础数据，所有库都拷贝一份
	+  允许字段冗余：这样有些字段就不用join去查询了
...
当你想在在淘宝买一样商品，此时商品只有一件库存，假如这个时候还有另一个人要买，那么如何解决是你买到还是另一个人买到的问题呢？这里肯定要用到事务，我们先从库存中取出商品的数量，然后插入订单，付款后插入付款表信息，然后更新商品数量。在这个过程中，锁可以对有限的资源进行保护，解决隔离和并发的矛盾
...
`Table_locks_waited`：出现表级锁定争用而发生等待的次数（即不能立即获取锁的次数，每等待一次锁值加1），此值越高，则说明存在较严重的表级锁争用竞争情况，实际工程中看此值就够了

**此外，`MyISAM`的读写锁调度是写优先，这也是`MyISAM`不适合作为主表的引擎的原因。因为执行写操作后，其他线程不能进行任何操作，大量的写操作会使查询很难得到锁，从而造成永远的阻塞**
...
- 尽可能让所有数据检索都通过索引来完成，避免索引失效使得行锁升级为表锁(Varchar类型不加单引号是重罪！)
...
- `Insert`操作：当写入一条记录时，分区层先打开并锁住所有的底层表，然后确定哪个分区接受这条记录，再将记录写入对应的底层表
- `Delete`操作：当删除一条记录时，分区层先打开并锁住所有的底层表，然后确定数据对应的分区，最后对相应的底层表进行删除操作
- `Update`操作：当更新一条数据时，分区层先打开并锁住所有的底层表，Mysql先确定待更新数据在哪个分区，然后取出数据并更新，再判断更新后的数据应该放在哪个分区，然后对相应的底层表进行写入操作，并对原数据所在的底层表进行删除操作

虽然**每次操作都会打开并锁住所有的底层表**，但这并不是说分区表在处理过程中是锁住全表的，如果存储引擎能够自己实现行级锁，如InnoDB，则会在分区层释放对应的表锁，这个加锁和解锁过程与普通InnoDB上的查询类似
...
+ 对于那些已经失去保存意义的数据，通常可以通过删除与那些数据有关的分区，很容易地达到删除数据的目的。相反地，在某些情况下，添加新数据的过程又可以通过为那些新数据专门增加一个新的分区的方法，很方便地实现
+ 一些查询可以得到极大的优化，这主要是借助于满足一个给定WHERE语句的数据可以只保存在一个或多个分区内，这样在查找时就不用查找其他剩余的分区
+ 涉及到例如SUM()和COUNT()这样聚合函数的查询，可以很容易地进行并行处理。这种查询的一个简单例子如 `SELECT salesperson_id, COUNT (orders) as order_total FROM sales GROUP BY salesperson_id；`, 通过“并行”，这意味着该查询可以在每个分区上同时进行，最终结果只需通过总和所有分区得到的结果(MapReduce思想)
...
+ 分区表中无法使用外键约束
+ MySQL数据库支持的分区类型为水平分区，并不支持垂直分区。因此，MySQL数据库的分区中索引是局部分区索引，即一个分区中既存放了数据又存放了索引，而全局分区是指数据放在各个分区中，但索引放在另外一个对象中
+ 目前MySQL不支持空间表类型和临时表类型进行分区、不支持全文索引
